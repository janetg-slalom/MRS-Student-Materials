...
/****** Object:  StoredProcedure [dbo].[Generate_USPollution_CO_Interaction_Model]    Script Date: 3/9/2017 12:45:23 PM ******/
SET ANSI_NULLS ON
GO
...
GO
 ...
 CREATE PROCEDURE [dbo].[Generate_USPollution_CO_Interaction_Model]   
 AS

 BEGIN
  BEGIN TRY 
		DROP TABLE [dbo].[Model_US_Pollution_CO]
	END TRY
	BEGIN CATCH
	END CATCH
	;

DECLARE @CO_Model table  (
    Model_Name varchar(30) not null default('CO rxLinMod ' + format(getdate(), 'yyyy.MM.dd', 'en-gb')) primary key,
...
CO_Model <- rxLinMod(formula = CO_Formula, data = df);

Trained_Model <- data.frame(payload = as.raw(serialize(CO_Model, connection=NULL)))'
;
	   

INSERT INTO @CO_Model(Model)
...
SELECT *
INTO dbo.Model_US_Pollution_CO
FROM @CO_Model;
