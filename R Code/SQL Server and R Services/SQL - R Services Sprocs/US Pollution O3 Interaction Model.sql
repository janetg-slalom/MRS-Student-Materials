...
/****** Object:  StoredProcedure [dbo].[Generate_USPollution_O3_Interaction_Model]    Script Date: 3/9/2017 12:45:49 PM ******/
SET ANSI_NULLS ON
GO
...
GO
 ...
 CREATE PROCEDURE [dbo].[Generate_USPollution_O3_Interaction_Model]   
 AS

 BEGIN
  BEGIN TRY 
		DROP TABLE [dbo].[Model_US_Pollution_O3]
	END TRY
	BEGIN CATCH
	END CATCH
	;

DECLARE @O3_Model table  (
    Model_Name varchar(30) not null default('O3 rxLinMod ' + format(getdate(), 'yyyy.MM.dd', 'en-gb')) primary key,
...
O3_Model <- rxLinMod(formula = O3_Formula, data = df);

Trained_Model <- data.frame(payload = as.raw(serialize(O3_Model, connection=NULL)))'
;
	   

INSERT INTO @O3_Model(Model)
...
SELECT *
INTO dbo.Model_US_Pollution_O3
FROM @O3_Model;
