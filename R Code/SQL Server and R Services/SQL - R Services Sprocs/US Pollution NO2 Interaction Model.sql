...
/****** Object:  StoredProcedure [dbo].[Generate_USPollution_NO2_Interaction_Model]    Script Date: 3/9/2017 12:48:15 PM ******/
SET ANSI_NULLS ON
GO
...
GO
 ...
 CREATE PROCEDURE [dbo].[Generate_USPollution_NO2_Interaction_Model]   
 AS

 BEGIN
  BEGIN TRY 
		DROP TABLE [dbo].[Model_US_Pollution_NO2]
	END TRY
	BEGIN CATCH
	END CATCH
	;

DECLARE @NO2_Model table  (
    Model_Name varchar(30) not null default('NO2 rxLinMod ' + format(getdate(), 'yyyy.MM.dd', 'en-gb')) primary key,
...
NO2_Model <- rxLinMod(formula = NO2_Formula, data = df);

Trained_Model <- data.frame(payload = as.raw(serialize(NO2_Model, connection=NULL)))'
;
	   

INSERT INTO @NO2_Model(Model)
...
SELECT *
INTO dbo.Model_US_Pollution_NO2
FROM @NO2_Model;
